...
1. **憲法最高性**：本憲法凌駕於所有其他開發實踐與口頭約定。任何與本憲法衝突的行為皆視為無效。
2. **修憲程序**：對本憲法的任何修改，必須經過正式的 PR (Pull Request) 審查，並附帶詳細的修改原因與對現有系統的影響評估 (Impact Assessment)。
3. **強制合規審查**：所有 PR 必須包含合規性檢查步驟，確保變更符合 ASPICE/ISO 標準及本憲法規範。
4. **複雜度控制**：任何新增的架構複雜度（如引入新工具、新資料表）必須在規格文件中提出充分的理由，並獲得架構師核准。
