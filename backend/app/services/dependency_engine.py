"""Dependency Engine service for graph and dependency helper queries."""
...
async def build_dependency_graph(project_id: uuid.UUID, db: AsyncSession) -> dict[str, list[str]]:
    """
    Build an adjacency list using normalized upstream->downstream semantics.
...
async def get_directly_dependent_specs(document_id: uuid.UUID, db: AsyncSession) -> list[uuid.UUID]:
    """Return document IDs directly dependent on the given upstream document."""

    upstream_items_result = await db.execute(select(SpecItem.id).where(SpecItem.document_id == document_id))
...
async def get_bu_scope(document_id: uuid.UUID, db: AsyncSession) -> uuid.UUID | None:
    """Return the BU scope node ID for a document (FR-041 filter input)."""
